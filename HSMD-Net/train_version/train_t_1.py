    ...
    with tqdm(total=len(train_loader)) as t:
        for (hazy, clean) in train_loader:
           ...
           # fake_out = DNet(output).mean()
            ...
            #adversarial_loss_co = torch.mean(real_out - img_out)
            smooth_loss_l1_co = F.smooth_l1_loss(img, real_out)
            perceptual_loss_co = loss_network(img, real_out)
            msssim_loss_co = -msssim_loss(img, real_out, normalize=True)
